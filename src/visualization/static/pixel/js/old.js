...
class CaravanXBlockchain {
  ...
  addCaravan(block, position, isNew = false) {
    ...
    if (isNew) {
      ...
      // Animate the caravan appearing with safety checks
      const entryAnimation = gsap.to(caravan, {
        x: pathPoint.x,
        duration: 3,
        ease: "power2.out",
        onComplete: () => {
          if (caravan && caravan.parent) {
            // Add flag waving animation
            this.addFlagWavingAnimation(hashFlag);

            // Add wheel rotation animation (in a real implementation)
  ...
  addTrader(txid, x, y, state = "waiting") {
    ...
    // Add idle animation with safety checks
    if (state === "waiting") {
      // Walking in place animation
      const walkingAnimation = gsap.to(trader, {
        y: trader.y - 3,
        duration: 0.4,
        repeat: -1,
        yoyo: true,
    ...
    try {
      ...
      if (this.state.caravans.length > 0 && this.effectsLayer) {
        ...
        gsap.to(highlight, {
          ...
          yoyo: true,
          onComplete: () => {
            if (this.effectsLayer && highlight && highlight.parent) {
        ...
        // Track for cleanup
        this.activeAnimations.add(highlight);
      }
    } catch (error) {
    ...
    try {
      ...
      if (this.state.caravans.length > 0) {
        ...
        const waitingTraders = this.state.traders
          .filter((trader) => trader && trader.state === "waiting")
    ...
    try {
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        gsap.to(particle, {
          ...
          onComplete: () => {
            if (this.effectsLayer && particle && particle.parent) {
  ...
  updateWeather() {
    try {
      ...
      if (!Array.isArray(this.weathers) && !this._localWeathers) {
        // allow for injection or default below
        this._localWeathers = ["clear", "cloudy", "rainy"];
      }
      ...
      // Determine possible weathers
      const weathers = Array.isArray(this.weathers)
        ? this.weathers
        : this._localWeathers;
      if (!weathers.includes(weather)) {
        console.warn("updateWeather: current weather not recognized", weather);
        this.state.weather = weathers[0];
      }
      ...
      // Final sanity before calling changeWeather
      const nextWeather = weathers[newIndex];
      if (typeof this.changeWeather !== "function") {
        console.error("updateWeather: changeWeather() is not defined");
        return;
      }
      if (typeof nextWeather !== "string") {
        console.error("updateWeather: invalid next weather", nextWeather);
        return;
      }
  ...
  createRainSplash(x, y) {
    ...
    try {
      ...
      gsap.to(splash, {
        ...
        duration: 0.5,
        onComplete: () => {
          if (this.effectsLayer && splash && splash.parent) {
      ...
      // Track for cleanup
      this.activeAnimations.add(splash);
    } catch (error) {
    ...
    try {
      ...
      if (this.state.traders && Array.isArray(this.state.traders)) {
        this.state.traders.forEach((trader) => {
          ...
          if (trader.state === "waiting" && Math.random() < 0.01) {
        ...
        switch (eventType) {
          ...
          case 2:
            ...
            if (this.state.miners.length > 0) {
              ...
              if (randomMiner && randomMiner.minerData) {
                this.showNotification(
          ...
          case 4:
            ...
            if (this.state.caravans.length > 0) {
              ...
              if (randomCaravan && randomCaravan.blockData) {
                this.showNotification(
  ...
  cleanup() {
    try {
      ...
      // Remove PixiJS containers and stage
      if (this.app) {
  ...
  gameLoop(delta) {
    try {
      ...
      if (this.state.camera) {
        ...
        // Force camera position to show content initially
        if (this.state.gameTime < 1 && this.worldInitialized) {
          console.log("Initial camera positioning at coordinates: 0,0");
        ...
        if (this.worldContainer) {
          ...
          // Log world container position for debugging
          if (this.state.gameTime < 1) {
            console.log(
              "World container positioned at:",
